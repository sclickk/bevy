...
impl BlobVec {
	...
	pub unsafe fn new(
		...
		if item_layout.size() == 0 {
			...
			let swap_scratch = NonNull::new(alloc::alloc::alloc(item_layout))
				.unwrap_or_else(|| alloc::alloc::handle_alloc_error(item_layout));
	...
	pub unsafe fn replace_unchecked(&mut self, index: usize, value: OwningPtr<'_>) {
		...
		core::ptr::copy_nonoverlapping::<u8>(value.as_ptr(), ptr, self.item_layout.size());
		self.len = old_len;
	}
...
impl Drop for BlobVec {
	fn drop(&mut self) {
		...
		if self.item_layout.size() > 0 {
			...
			unsafe {
				alloc::alloc::dealloc(self.swap_scratch.as_ptr(), self.item_layout);
			}
		}
		let array_layout =
			array_layout(&self.item_layout, self.capacity).expect("array layout should be valid");
