use crate::{
	...
	entity::Entity,
	query::{
		QueryCombinationIter, QueryEntityError, QueryItem, QueryIter, QueryManyIter, QuerySingleError,
	...
	pub fn iter_combinations<const K: usize>(
		...
		unsafe {
			self
				.state
				.as_readonly()
				.iter_combinations_unchecked_manual(self.world, self.last_change_tick, self.change_tick)
		}
	}
	...
	pub fn for_each<'this>(&'this self, f: impl FnMut(ROQueryItem<'this, Q>)) {
		...
		unsafe {
			self
				.state
				.as_readonly()
				.for_each_unchecked_manual(self.world, f, self.last_change_tick, self.change_tick);
		};
	}
	...
	pub fn get_single(&self) -> Result<ROQueryItem<'_, Q>, QuerySingleError> {
		...
		unsafe {
			self
				.state
				.as_readonly()
				.get_single_unchecked_manual(self.world, self.last_change_tick, self.change_tick)
		}
	}
