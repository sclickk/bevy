...
impl RenderGraph {
	...
	pub fn input_node(&self) -> Option<&NodeState> {
		self
			.input_node
			.and_then(|id| self.get_node_state(id).ok())
	}
	...
	pub fn remove_node(
		...
		if let Some(id) = self.node_names.remove(&name) {
			if let Some(node_state) = self.nodes.remove(&id) {
				...
				for input_edge in node_state.edges.input_edges().into_iter() {
					match input_edge {
				...
				for output_edge in node_state.edges.output_edges().into_iter() {
					match output_edge {
						Edge::SlotEdge {
							output_node: _,
							output_index: _,
	...
	pub fn get_node_mut<T>(&mut self, label: impl Into<NodeLabel>) -> Result<&mut T, RenderGraphError>
	where
		T: Node,
	{
		self
			.get_node_state_mut(label)
			.and_then(|n| n.node_mut())
	}
		...
		match *edge {
			Edge::SlotEdge {
				...
				let input_slot = input_node_state
					...
					.ok_or(RenderGraphError::InvalidInputNodeSlot(SlotLabel::Index(
						input_index,
					)))?;
